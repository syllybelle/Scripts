...
study_api_client_csv  <- "viedoc-export/studies.csv" 
...
nest_depth <- 2 # 0: export_file_location/name.file. 3:  file_location/study_ref/yymmddhhmm/name.file
exclude_download_name <- TRUE # export_file_location/study_ref/yymmddhhmm_{STUDY_GUID__YYYYMMDD_HHmmss}.file  
exclude_generated_name <- FALSE # export_file_location/study_ref/{STUDY_GUID__YYYYMMDD_HHmmss}.file 
...
build_output_path <- function(
    ...
    if (!isTRUE(exclude_download_name) && !is.null(download_name) && !is.na(download_name) && nzchar(download_name)) {
        ...
        base <- if (nzchar(file_prefix)){paste(file_prefix, dl_name, sep = "_") } else dl_name
    }
    ...
    tryCatch(
      {
        ...
        if (http_error(resp)) {
          logerror("HTTP error %s: %s", status_code(resp),
      ...
      if (!"exportId" %in% names(resp_parsed)) {
        logerror("Response did not contain 'exportId': %s",
                 paste(names(resp_parsed), collapse = ", "))
        return(NULL)
      }
...
get_data_export_task <- function(api_url, auth, export_id, task='status'){
  url <- paste(api_url, "/clinic/dataexport/", task, "?exportId=", export_id, sep = "")
    ...
    response <- GET(
        ...
        add_headers(Authorization = auth))
    if (http_error(response)) {
      logerror("HTTP error %s: %s", status_code(response),
              content(response, "text", encoding = "UTF-8"))
      return(NULL)
    }
    else {
  ...
  tryCatch(
      {
        ...
        if (!"exportStatus" %in% names(resp_parsed)) {
          logerror("Response did not contain 'exportStatus': %s",
                   paste(names(resp_parsed), collapse = ", "))
          return(NULL)
        }
    ...
    tryCatch(
      {
        ...
        if (is.null(response)) return(list(content_raw = NULL, download_name = NULL))
...
for (i in seq_len(nrow(studies))) {
  ...
  apiURL      <- studies$apiURL[i]
  tokenURL    <- studies$tokenURL[i]
  print(study_ref)
  print(clientId)
  print(clientSecret)
  print(apiURL)
  ...
  loginfo("Processing study: %s", study_ref)
  ingest_study_data(
    study_ref = study_ref,
    clientId = clientId,
    ...
    maximum_wait_time_in_s = maximum_wait_time_in_s,
    check_every_n_s = check_every_n_s,
